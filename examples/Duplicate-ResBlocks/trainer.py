...
from cg.loader import load_dataset_and_model
from cg.train_utils import seed_everything, calculate_pos_weight
from cg.models.duplicate_resblock_resnet import duplicate_resblock_resnet50
...
def parse_arguments():
    ...
    # duplicate ResNet args
    parser.add_argument('--duplicate_copies', type=int, default=2)
    parser.add_argument('--duplicate_layer', type=int, default=4)
    parser.add_argument('--duplicate_block', type=int, default=2)
...
def main():
    ...
    '''Fixed hparams
    '''
    finetune_layer_start = f"layer{args.duplicate_layer}.{args.duplicate_block}"
    arch_name = 'dup-resnet50'
    
    if args.exp_name_prefix is not None:
        exp_name = args.exp_name_prefix
    else:
        exp_name = f"{args.dset_name}_{args.task}_{arch_name}"
    if args.use_all_data:
        exp_name += '_all-data'
    if args.x2c_from_x2y_ckpt is not None:
        exp_name += '_ft-x2y'
    exp_name += f'_ft-{finetune_layer_start}+_{args.duplicate_copies}x'
    if args.seed is not None:
    ...
    model_kwargs = {'duplicate_layer': args.duplicate_layer, 'duplicate_block': args.duplicate_block, 
    ...
    dl_train, dl_valid, dl_test, model = load_dataset_and_model(
        ...
        arch_name, args.x2c_from_x2y_ckpt, args.bsize, model_kwargs=model_kwargs)
    ...
    for name, p in model.named_parameters():
        if name.startswith(finetune_layer_start):
            finetune_flag = True
