...
def parse_arguments():
    ...
    # optional
    parser.add_argument('--CAV_attr_path', type=str, default=None)
    parser.add_argument('--random_CAV_attr_path', type=str, default=None)
...
def main():
    ...
    if args.CAV_attr_path is not None:
        CAV_attrs = np.load(args.CAV_attr_path)
    else:
        for layer_name in tqdm(layers, leave=True, desc="Layers: "):
            ...
            for xs, ys in tqdm(dl, leave=False, desc="Eval dset: "):
                ...
                CAV_attrs.append(attr)

            CAV_attrs = np.concatenate(CAV_attrs, axis=0)

    if args.random_CAV_attr_path is not None:
        random_CAV_attrs = np.load(args.random_CAV_attr_path)
    else:
        for layer_name in tqdm(layers, leave=True, desc="Layers: "):
            random_CAV_attrs = []
            for xs, ys in tqdm(dl, leave=False, desc="Eval dset: "):
                ...
                attr = tcav.attribute(xs, layer_name, args.mode, target=ys, use_random=True)
                attr = attr.detach().cpu().numpy()
                random_CAV_attrs.append(attr)

            random_CAV_attrs = np.concatenate(random_CAV_attrs, axis=0)
